 ...
 *  in the recips array.
 *
 *  For each recipient whose state is HV_TO_BE_SENT, the hypervisor
 *  attempts to send that tile the specified message.  In order to
 *  successfully receive the message, the receiver must be a valid tile
 *  to which the sender has access and must have sufficient free buffer
 *  space.  (The hypervisor guarantees that each tile which has called
 *  hv_register_message_state() will be able to buffer one message from
 *  every other tile which can legally send to it; more space may be
 *  provided but is not guaranteed.)  If an invalid tile is specified,
 *  the recipient's state is set to HV_BAD_RECIP; this is a permanent
 *  delivery error.  If the message is successfully delivered to the
 *  recipient's buffer, the recipient's state is set to HV_SENT.
...
#define HV_MSG_EDAC 0x2         /**< Message is for cache error */

/** Correctable cache error types */
#define HV_CE_L2_D_ECC_ERROR      0x1	/**< L2$ data ECC error */
#define HV_CE_L2_OVERFLOW_ERROR   0x2	/**< L2$/L3$ multiple simultaneous */
#define HV_CE_L1_D_TAG_ERROR      0x3	/**< L1D$ tag parity error */
#define HV_CE_L1_D_DATA_ERROR     0x4	/**< L1D$ data parity error */
#define HV_CE_L1_D_OVERFLOW_ERROR 0x5	/**< L1D$/TLB multiple simultaneous */
#define HV_CE_L1_I_ERROR          0x6	/**< L1I$ parity error */


/** Cache error messages are delivered via the messaging mechanism,
 *  but have a message source of HV_MSG_EDAC.  The message is formatted
 *  as an HV_EdacMsg structure.
 */
typedef struct
{
  __hv32 edactype; /**< Code identifying cache error type (HV_CE_xxx) */
  __hv64 edacinfo; /**< Additional information identifying cache error.
                        For L2$ and L1D$ errors, it stores the physical
                        address which causes such errors; for L1I$ errors,
                        it stores the index to the I$. */
  HV_Coord tile;   /**< Tile which encounters cache error */
} HV_EdacMsg;
 ...
 * as a result of a device interrupt, this value is HV_MSG_INTR; for
 * messages sent as a correctable cache error, this value is HV_MSG_EDAC.
 */
