...
## A token composed of a type (parenthesis, etc.) and, depending on
# type, a value (the identifier string).
class Token:
...
def parse_number():
    ...
    if (file[index] == '0' and
        ...
        (file[index + 1] == 'x' or file[index + 1] == 'X') and
...
def parse_identifier():
    ...
    while (index < len(file) and
           (file[index].isalnum() or
            file[index] == '.' or
            file[index] == '_' or
            file[index] == '/')):  # ISSUE: could consume comment blocks.
        index += 1
