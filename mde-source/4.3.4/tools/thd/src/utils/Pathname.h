...
public:
  ...
  // --- operator overloads ---
public:
  /** Operator overload() for concatenate() */
  Pathname operator+(const Pathname& pathname) const
  {
    return Pathname(concatenate(*this, pathname));
  }
...
public:
  ...
  void get_names(Array<std::string>& names) const;
  ...
  bool get_child_names(Array<std::string>& names) const;
...
public:
  /** Stores sorted list of subdirectory/file names in supplied list.
      Returns number of directories/files found.
      Returns <0 if directory cannot be accessed.
      The directories/files flags indicate which name(s) to include.
  */
  int
  directory_list(Array<Pathname>& list,
                 bool directories = true, bool files = true) const;
