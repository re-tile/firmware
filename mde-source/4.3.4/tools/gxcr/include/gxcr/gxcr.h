 ...
 * a description of the operations to be performed on data, such as which
...
/**
 * @addtogroup gxcr_error
 * @{
 */

/** Error codes returned by gxcr_ API calls. */
#else
...
/** Length, in bytes, of an AES-GCM Initialization Vector */
#define GXCR_AES_GCM_IV_SIZE 12

/** Length, in bytes, of 4 octet 'salt' part of an AES-GCM IV */
#define GXCR_AES_GCM_SALT_SIZE 4
...
/** Length, in bytes, of a AES-GCM digest */
#define GXCR_AES_GCM_DIGEST_SIZE  12
...
typedef enum {
  ...
  GXCR_CIPHER_AES_GCM_128,
  /** AES-GCM with 192-bit key */
  GXCR_CIPHER_AES_GCM_192,
  /** AES-GCM with 256-bit key */
  GXCR_CIPHER_AES_GCM_256,
  /** DES-ECB */
  GXCR_CIPHER_DES_ECB,
  /** DES-CBC */
  GXCR_CIPHER_DES_CBC,
 ...
 * See Appendix D of the I/O Device Guide (UG404) for more detail.
 */
typedef union
{
  ...
  struct
  {
    ...
    uint32_t B : 1;
    /** Indicates that a checksum is appended to the result packet. */
    uint32_t C : 1;
...
typedef struct gxcr_context {
  ...
  /** Input source data block size, be used to perform data length check. */
  uint8_t src_data_block_size;

#else
  /** Internal data, not for direct access by user. */
  uint32_t internal0;
 ...
 * precalculation step with gxcr_hmac_precalc().
 * @param digest_len Length of the digest field for this algorithm.
 */
extern size_t
hmac_precalc_calc_metadata_bytes(int digest_len);
 ...
 * @param crypto_context An initialized gxcr context.
 * @param scratch_mem Pointer to scratch memory to be used by the shim.
 *   Memory must be 32-bit aligned and registered with the MiCA shim via
 *   gxio_mica_register_page().
 * @param scratch_mem_len Length of provided precalc metadata memory, in bytes.
 *   The value should be at least max(hmac_key_len, data block size) +
 *   metadata size calculated by function: hmac_precalc_calc_metadata_bytes(.).
 ...
 */
extern int
gxcr_do_op(gxio_mica_context_t* mica_context,
           gxcr_context_t* crypto_context,
...
/** Run the first operation of a successive operation indicated by
 * the initialized gxcr_context.  This function blocks until the shim
...
/** Run the middle operations of a successive operation indicated by
 * the initialized gxcr_context.  This function blocks until the shim
 * has completed the operation.
 * @param mica_context An initialized MiCA context.
 * @param crypto_context An initialized gxcr context.
...
/** Run the last operation of a successive operation indicated by
 * the initialized gxcr_context.  This function blocks until the shim
 * has completed the operation.
 * @param mica_context An initialized MiCA context.
 * @param crypto_context An initialized gxcr context.
 ...
 */
static __inline void*
gxcr_context_key(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline int
gxcr_context_key_len(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline void*
gxcr_context_iv(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline int
gxcr_context_iv_len(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline void*
gxcr_context_digest0(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline int
gxcr_context_digest0_len(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline void*
gxcr_context_digest1(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline int
gxcr_context_digest1_len(gxcr_context_t* crypto_context)
{
 ...
 */
static __inline void*
gxcr_digest_count(gxcr_context_t* crypto_context)
{
...
/** Function to convert a GXCR error code to a string.
