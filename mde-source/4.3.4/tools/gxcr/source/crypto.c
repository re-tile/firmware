...
#include <gxcr/tokens/hmac_precalc.h>

#include "common.h"

/* Token for bulk crypto/digest processing. */
extern gxcr_token_info_t bulk_crypto_digest_token;
extern gxcr_token_info_t bulk_successive_token;

static int
setup_token_info(gxcr_token_info_t* token_info,
                 gxcr_cipher_t cipher, gxcr_digest_t digest, int hmac_mode,
                 int successive, unsigned char* pmetadata)
{
  gxcr_token_info_t* token_template = successive ? &bulk_successive_token :
    &bulk_crypto_digest_token;
  ...
  if (hmac_mode && digest != GXCR_DIGEST_NONE)
  {
    ...
    case GXCR_DIGEST_SHA_256:
      token_info->digest0_len = GXCR_SHA2_224_256_DIGEST_FIELD_SIZE;
      token_info->digest1_len = GXCR_SHA2_224_256_DIGEST_FIELD_SIZE;
      break;
    case GXCR_DIGEST_SHA_384:
    case GXCR_DIGEST_SHA_512:
      token_info->digest0_len = GXCR_SHA2_384_512_DIGEST_FIELD_SIZE;
      token_info->digest1_len = GXCR_SHA2_384_512_DIGEST_FIELD_SIZE;
      break;
    default:
      return GXCR_OPERATION_NOT_SUPPORTED;
    }
                            ...
                            int successive, int context_length)
{
  ...
  case GXCR_CIPHER_AES_ICM_128:
    ccw->crypto_algorithm = 5;
    ccw->crypto_mode_plus_feedback = 7;
    break;
  case GXCR_CIPHER_AES_ICM_192:
    ccw->crypto_algorithm = 6;
    ccw->crypto_mode_plus_feedback = 7;
...
size_t 
gxcr_calc_context_bytes(gxcr_cipher_t cipher, gxcr_digest_t digest,
                    ...
                    void* hmac_key, int hmac_key_len)
{
  ...
  void* data_mem;
  int data_mem_len;
  char key_copy[128];            // max block_size
  ...
  data_mem_len = (hmac_key_len > block_size) ? hmac_key_len : 
    block_size;
  ...
  gxcr_memclr(data_mem, block_size);
  gxcr_memcpy(data_mem, hmac_key, hmac_key_len);
  ...
  if (hmac_key_len > block_size)
  {
    ...
    // to finish the hash
    hmac_precalc_setup(metadata_mem, metadata_mem_len, ccw->hash_algorithm, 1,
                       digest_field_size, hmac_key_len);

    // Run the hmac precalculation token.
    hmac_precalc_process(mica_context, metadata_mem, metadata_mem_len,
                         data_mem, hmac_key_len);

    // Put a zero-padded copy of the resulting digest into the data memory.
    gxcr_memclr(data_mem, block_size);
    gxcr_memcpy(data_mem, hmac_precalc_digest(metadata_mem), digest_len);
  }
  ...
  gxcr_memcpy(key_copy, precalc_data, block_size);
  ...
  hmac_precalc_setup(metadata_mem, metadata_mem_len, ccw->hash_algorithm, 0,
                     digest_field_size, block_size);
  ...
  gxcr_memcpy(context_metadata_mem + context_token_info.digest0_offset,
         hmac_precalc_digest(metadata_mem), digest_field_size);

  // Run outer digest, XOR each byte with 0x5c.
  gxcr_memcpy(precalc_data, key_copy, block_size);
  ...
  gxcr_memcpy(context_metadata_mem + context_token_info.digest1_offset,
         hmac_precalc_digest(metadata_mem), digest_field_size);
                     ...
                     void* ghash_key, int ghash_key_len)
{
  ...
  void* data_mem;
  uint32_t* digest0;
  gxcr_context_t ghash_ctx;
  int ghash_ctx_size;
  int retval;

  switch(ghash_key_len) {
  case GXCR_AES_128_KEY_SIZE:
    cipher = GXCR_CIPHER_AES_ECB_128;
    break;
  case GXCR_AES_192_KEY_SIZE:
    cipher = GXCR_CIPHER_AES_ECB_192;
    break;
  case GXCR_AES_256_KEY_SIZE:
    cipher = GXCR_CIPHER_AES_ECB_256;
    break;
  default:
    return GXCR_OPERATION_NOT_SUPPORTED;
  }

  block_size = GXCR_AES_IV_SIZE;

  // Calc. the context's size.
  ghash_ctx_size = gxcr_calc_context_bytes(cipher, GXCR_DIGEST_NONE,
                                           0, 0);

  // Check if there is enough scratch memory to calc the ghash's H word.
  if (ghash_ctx_size + 2 * block_size > scratch_mem_len)
    return GXCR_ERR_INVAL_MEMORY_SIZE;

  // Make a context.
  retval = gxcr_init_context(&ghash_ctx, scratch_mem, ghash_ctx_size,
                             cipher, GXCR_DIGEST_NONE, ghash_key,
                             NULL, 1, 0, 0);
  if (retval)
    return retval;

  // The new metadata pointer.
  data_mem = scratch_mem + ghash_ctx_size;

  // Clear the metadata area.
  gxcr_memclr(data_mem, 2 * block_size);

  retval = gxcr_do_op(mica_context, &ghash_ctx, data_mem,
                      data_mem + block_size,  block_size);
  if (retval)
    return retval;

  // Copy the encrypt result into digest0 in the CR.
  digest0= (uint32_t*)(context_metadata_mem +
                       context_token_info.digest0_offset);

  digest0[0] = __builtin_bswap32(((uint32_t*)data_mem)[4]);
  digest0[1] = __builtin_bswap32(((uint32_t*)data_mem)[5]);
  digest0[2] = __builtin_bswap32(((uint32_t*)data_mem)[6]);
  digest0[3] = __builtin_bswap32(((uint32_t*)data_mem)[7]);

  return 0;
}

static void
backwards_copy(unsigned char* dst, unsigned char* src, size_t len)
{
                    ...
                    void* xcbc_key, int xcbc_key_len)
{
  ...
  */
  ...
  backwards_copy(context_metadata_mem + context_token_info.digest1_offset,
                 enck, GXCR_AES_128_KEY_SIZE);
  ...
  backwards_copy(context_metadata_mem + context_token_info.digest0_offset,
                 enck, GXCR_AES_128_KEY_SIZE);
...
int 
gxcr_hmac_precalc(gxio_mica_context_t* mica_context,
...
static int
__gxcr_init_context(gxcr_context_t* crypto_context,
                    ...
                    int hmac_mode, int successive)
{
  ...
  gxcr_memclr(metadata_mem, metadata_mem_len);

  token_info = (gxcr_token_info_t*)&crypto_context->token_info;
  res = setup_token_info(token_info, cipher, digest, hmac_mode, successive,
                         metadata_mem);
  ...
  crypto_context->src_data_block_size = 1;
  // Set the encrypt src data block size.
  switch (cipher)
  {
  case GXCR_CIPHER_NONE:
  case GXCR_CIPHER_NONE_NOCOPY:
  ...
  case GXCR_CIPHER_AES_ECB_256:
  case GXCR_CIPHER_AES_ICM_256:
    crypto_context->src_data_block_size = 16;
    break;
  ...
  case GXCR_CIPHER_3DES_CBC:
  case GXCR_CIPHER_3DES_ECB:
    crypto_context->src_data_block_size = 8;
    break;
  ...
  if (key_dst && key)
    gxcr_memcpy(key_dst, key, gxcr_context_key_len(crypto_context));

  iv_dst = gxcr_context_iv(crypto_context);
  if (iv_dst && iv)
    gxcr_memcpy(iv_dst, iv, gxcr_context_iv_len(crypto_context));
...
int
gxcr_init_context(gxcr_context_t* crypto_context,
                  void* metadata_mem, int metadata_mem_len,
                  gxcr_cipher_t cipher, gxcr_digest_t digest,
                  unsigned char* key, unsigned char* iv,
                  int encrypt, int cipher_then_digest,
                  int hmac_mode)
{
  if (metadata_mem_len < gxcr_calc_context_bytes(cipher, digest, hmac_mode, 0))
    return GXCR_ERR_INVAL_MEMORY_SIZE;
  ...
  return __gxcr_init_context(crypto_context, metadata_mem, metadata_mem_len,
                             cipher, digest, key, iv, encrypt,
                             cipher_then_digest, hmac_mode, 0);
}


int
gxcr_init_context_successive_crypto(gxcr_context_t* crypto_context,
                                    void* metadata_mem, int metadata_mem_len,
                                    gxcr_cipher_t cipher, unsigned char* key,
                                    unsigned char* iv, int encrypt)
{
  ...
  memset(crypto_context, 0, sizeof(*crypto_context));
  crypto_context->setup = bulk_successive_setup;
  crypto_context->start = bulk_successive_process_start;

  return __gxcr_init_context(crypto_context, metadata_mem, metadata_mem_len,
                             cipher, GXCR_DIGEST_NONE, key, iv, encrypt,
                             0, 0, 1);
}


int
gxcr_init_context_successive_digest(gxcr_context_t* crypto_context,
                                    void* metadata_mem, int metadata_mem_len,
                                    gxcr_digest_t digest)
{
  ...
  return __gxcr_init_context(crypto_context, metadata_mem, metadata_mem_len,
                             GXCR_CIPHER_NONE_NOCOPY, digest, NULL, NULL, 0,
                             0, 0, 1);
}
...
static int 
gxcr_do_op_stage(gxio_mica_context_t* mica_context, 
                 gxcr_context_t* crypto_context,
                 void* src, void* dst, int len, int stage)
{
  int result = crypto_context->start(mica_context, crypto_context, src, len,
                                     dst, len, stage);
...
int 
gxcr_do_op(gxio_mica_context_t* mica_context, gxcr_context_t* crypto_context,
...
int 
gxcr_start_op(gxio_mica_context_t* mica_context,
              gxcr_context_t* crypto_context,
              void* src, void* dst, int len)
{
...
int 
gxcr_do_init_op(gxio_mica_context_t* mica_context,
                gxcr_context_t* crypto_context,
                void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
int 
gxcr_do_update_op(gxio_mica_context_t* mica_context,
                  gxcr_context_t* crypto_context,
                  void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
int 
gxcr_do_final_op(gxio_mica_context_t* mica_context,
                 gxcr_context_t* crypto_context,
                 void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
int 
gxcr_start_init_op(gxio_mica_context_t* mica_context,
                   gxcr_context_t* crypto_context,
                   void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
int 
gxcr_start_update_op(gxio_mica_context_t* mica_context,
                     gxcr_context_t* crypto_context,
                     void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
int 
gxcr_start_final_op(gxio_mica_context_t* mica_context,
                    gxcr_context_t* crypto_context,
                    void* src, void* dst, int len)
{
  return gxcr_do_op_stage(mica_context, crypto_context, src, dst, len, 
...
__inline uint32_t
gxcr_result_packet_length(gxcr_result_token_t* res)
{
...
__inline uint32_t
gxcr_result_error(gxcr_result_token_t* res)
{
  gxcr_result_token_t res_mem = { .word[0] = le32_to_cpu(res->word[0]) };
...
const char*
gxcr_strerror(int error)
{
  switch (error)
  {
  case GXCR_OPERATION_NOT_SUPPORTED:
    return "Unsupported Cipher/digest combination";
  case GXCR_BAD_PARAM:
    return "Invalid parameter";
  case GXCR_ERR_INVAL_MEMORY_SIZE:
    return "Invalid memory size";
  case GXCR_ERR_INVAL_TOKEN_SIZE:
