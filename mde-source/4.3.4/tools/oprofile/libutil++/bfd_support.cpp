...
/** Directory to use when looking for separate debuginfo files.
    Example: /usr/lib/debug
 */
static string g_debuginfo_directory;

/** Sets directory to use when looking for separate debuginfo files. */
void set_debuginfo_directory(const string& path) {
	g_debuginfo_directory = path;
}

/** Parent "root" directory to use when looking for separate debuginfo files.
    Example: (empty string, means use debuginfo_dir path as-is)
    Example: /my/TileraMDE/installation/tile
 */
static string g_debuginfo_root_directory;

/** Sets parent "root" directory to use when looking for separate debuginfo files. */
void set_debuginfo_root_directory(const string& path) {
	g_debuginfo_root_directory = path;
}

bool find_separate_debug_file(bfd * ibfd, string const & filepath_in, 
                              string & debug_filename, extra_images const & extra)
{
	cverb << vbfd << "looking for separate debug file for: " << filepath_in << endl;
	...
	cverb << vbfd << "looking for separate debug file matching " << basename 
	...
	// Initialize debuginfo directory paths, if needed.
	if (g_debuginfo_directory.empty()) {
		g_debuginfo_directory = DEBUGDIR;
	}
	cverb << vbfd << "debuginfo directory = " << g_debuginfo_directory << endl;

	if (! g_debuginfo_root_directory.empty()) {
		cverb << vbfd << "debuginfo root directory = " << g_debuginfo_root_directory << endl;
        }

	// Strip debuginfo root prefix, if one was specified
	string filedir_noroot = 
		(! g_debuginfo_root_directory.empty() && filedir.find(g_debuginfo_root_directory) == 0) ?
		filedir.substr(g_debuginfo_root_directory.size()) :
		filedir;

	string first_try(filedir + ".debug/" + basename);
	string second_try(g_debuginfo_root_directory + g_debuginfo_directory +
			      filedir_noroot + basename);
	...
	cverb << vbfd << "found separate debug file for: " << filepath_in
	      << " = " << debug_filename << endl;
...
/* HACK: manually apply patch that fixes bug
   when separate debuginfo file doesn't include copy of symbol data.
   Source: http://marc.info/?l=oprofile-list&m=132639666002027
*/
