...
op_bfd::op_bfd(string const & fname, string_filter const & symbol_filter,
	...
	:
	filename(fname),
// Tilera: We want to be able to specify a directory for the samples without
// using as a prefix for all executables.  The command line argument
// "archive:" does both and there is no way to specify at a finer level.  The
// minimal way to disable this feature for now.
#if 0
...
/** Converts file position offset to PC (virtual memory address).
    Offset is assumed to be a file position in some section defined by
	this bfd. We locate the section that contains that file position,
    and return the corresponding vma. If the offset does not fall in any
	section defined by this bfd, we return zero. */    
bfd_vma op_bfd::offset_to_pc(bfd_vma offset) const
{
	// Note: modification of original OProfile method,
	// to remove assumption that sect->filepos is monotonically ascending
	asection const * sect = ibfd.abfd->sections;

	for (; sect; sect = sect->next) {
		if (offset >= bfd_vma(sect->filepos) &&
		    offset < bfd_vma(sect->filepos + sect->size)) {
