...
string op_replace_proc_self_exe_filename(std::string const& tail)
{
	char buf[PATH_MAX+1];
	const int n = readlink("/proc/self/exe", buf, sizeof(buf));
	if (n < 0)
		throw std::out_of_range("Unexpected long pathname");
	string path(buf,0,n);
	return op_dirname(path) + "/" + tail;
}
