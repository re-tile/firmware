...
static void get_install_path(char* buf, size_t size, const char* tail)
{
	int i, n;

	if (tail[0] != '/' && tail[0] != '\0') {
		fprintf(stderr,
			"Cannot handle relative paths in 'get_install_path'.");
		exit(EXIT_FAILURE);
	}
	...
	if (n < 0) {
		fprintf(stderr, "Failure in 'readlink(\"/proc/self/exe\")'\n");
		exit(EXIT_FAILURE);
	}
	...
	fprintf(stderr, "Cannot get install dir from 'exe' path '%s'.", buf);
	exit(EXIT_FAILURE);
}
...
static char *build_fn(const char *cpu_name, const char *fn)
{
	...
	if (dir == NULL) {
		static char op_install_dir[PATH_MAX + 1];
		get_install_path(op_install_dir, PATH_MAX, "/share/oprofile");
		dir = op_install_dir;
	}
