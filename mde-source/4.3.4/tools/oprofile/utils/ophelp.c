			...
			struct op_event const * event)
{
	...
	// When callgraph sampling is enabled, scale min count accordingly
	// to account for extra overhead in capturing each sample.
	// This is somewhat arbitrary, but the range we shoot for is:
	// - minimum scaling of 2  when callgraph_depth is 1
	// - maximum scaling of 15 when callgraph depth is 20 or above.
	int callgraph_min_count_scale =
		(callgraph_depth == 0) ? 1 : (2 + callgraph_depth * 65 / 100);
	if (callgraph_min_count_scale > 15)
		callgraph_min_count_scale = 15;
	...
	if (pev->count < min_count)
	{
		...
		if (callgraph_depth > 0)
		{
			fprintf(stderr, "Note: actual event minimum is %d. "
				        "It has been scaled by %d to %d,\n"
				        "to reflect overhead of specified callgraph depth of %d.\n",
				event->min_count,
				callgraph_min_count_scale,
				min_count,
				callgraph_depth);
		}
		exit(EXIT_FAILURE);
...
int main(int argc, char const * argv[])
{
	...
	if (cpu_string) {
		...
		/** Default to appropriate chip type for current MDE build. */
		cpu_type = op_get_cpu_number("tile/" CHIP_ARCH_NAME);
	}
