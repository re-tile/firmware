	...
	image_error & error, bool fixup) const
{
	...
	// See if the image can be found locally.
	string const image = locate_image(image_name, error, fixup);

	// Now see if it can be found via the -p search path.
	string const base = op_basename(image);
	vector<string> result = find(base);

	// The -p location "wins" over the local copy, if any:
	if (result.empty() && error != image_not_found)
		return image;
