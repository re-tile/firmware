				  ...
				  string const & root_path)
{
	...
	for ( ; ip_it != image_path.end(); ++ip_it) {
		// Hack: Make "opreport/tile-opreport -p" can specify absolute
		// paths without archiving those "image" in archive path using
		// oparchive. 0.9.6 oparchive generate archive and ** storing
		// "images" ** in the archive path (e.g. oparchive -p
		// /boot,/lib,/root/myprogram -o), then opreport wll specify
		// original "image" paths (e.g. opreport -p /boot,/lib...).
		// We do not use oparchive for x86 due to the tile-monitor
...
#else
		if (!is_directory(*ip_it)) {
			cerr << *ip_it << " isn't a valid directory\n";
#endif
			ok = false;
		}
	}
	...
	spec.extra_found_images.populate(image_path, spec.get_archive_path(),
					 root_path);
#else
	spec.extra_found_images.populate(image_path, "", "");
#endif
