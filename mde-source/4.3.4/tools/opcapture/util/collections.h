...
public:
  ...
  /** Sorts array using operator<() definition for its elements. */
  void
  sort()
  {
  ...
  /** Sorts array using specified compare function for its elements. */
  void
  sort(bool (*compare)(const T&, const T&))
  {
...
/** Set (resizable unique array) class, with sorted-order storage.
    Operations:
     clear       -- removes all items
