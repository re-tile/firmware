...
// OProfile includes
// libpp
#include "parse_filename.h" // parsed_filename, parse_filenames(sample_file_path)
...
private:
  ...
  /** Cached event interval. */
  long m_event_interval;
...
public:
  ...
  /** Constructor */
  SampleFilePathname(const std::string& path);
...
{
  out << sample_file_pathname.get_pathname();
  return out;
}
