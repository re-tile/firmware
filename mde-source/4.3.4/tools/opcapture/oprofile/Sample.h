 ...
 * accumulated value.
 * The pair of SampleLocations includes the
 * "from" (caller) location and "to" (callee) location.
 */
class Sample {
...
protected:
  ...
  SampleLocation m_from_location;

  /** "to" sample location. */
  SampleLocation m_to_location;
...
public:
  /** Equality comparison. */
  bool
  equals(const Sample& s) const
  {
    return (m_from_location == s.m_from_location &&
            m_to_location == s.m_to_location &&
            m_value == m_value);
  }

  /** Operator overload, equivalent to equals(). */
  bool
  operator==(const Sample& s) const 
  {
    return equals(s);
  }
 ...
 public:
  ...
  void
  increment_value_by (odb_value_t inc)
  {
    m_value += inc;
  }
  ...
  bool
  is_callgraph_sample() const
  {
    return m_is_callgraph_sample;
  }
