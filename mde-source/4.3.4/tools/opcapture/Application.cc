...
Pathname Application::DEFAULT_OPROFILE_DATA_PATHNAME =
  Pathname("/var/lib/oprofile/samples/current");
...
bool Application::s_show_sample_files = false;

/** Whether to print sample files read. */
bool Application::show_sample_files() { return s_show_sample_files; }


/** Whether to print sample files skipped. */
bool Application::s_show_sample_files_skipped = false;

/** Whether to print sample files skipped. */
bool Application::show_sample_files_skipped() { return s_show_sample_files_skipped; }
...
bool Application::s_show_sample_details = false;

/** Whether to print details of samples read from sample files. */
bool Application::show_sample_details() { return s_show_sample_details; }


/** Whether to print symbol files read. */
bool Application::s_show_symbol_files = false;

/** Whether to print symbol files read. */
bool Application::show_symbol_files() { return s_show_symbol_files; }


/** Whether to print symbols read from symbol files. */
bool Application::s_show_symbols = false;

/** Whether to print symbols read from symbol files. */
bool Application::show_symbols() { return s_show_symbols; }


/** Whether to validate symbol binary-search lookup. */
bool Application::s_validate_symbol_lookup = false;

/** Whether to validate symbol binary-search lookup. */
bool Application::validate_symbol_lookup() { return s_validate_symbol_lookup; }
...
int Application::process_argument(const char* arg, int& n, int argc, char** argv)
{
  ...
  else if (starts_with(arg, "-s") || starts_with(arg, "--symbols"))
  {
    Application::s_show_symbols = true;
  }
  ...
  else if (starts_with(arg, "-B") || starts_with(arg, "--validate-symbol-lookup"))
  {
    Application::s_validate_symbol_lookup = true;
  }
  ...
  else
  {
    ...
    {
      if (m_verbose)
        printf("Profiling data archive specified: %s\n", arg);
      m_profile_data_pathnames.add(Pathname(arg));
    }
    else
    {
      printf("Multiple profile archive directories specified.\n"
             "Currently only one archive can be specified.\n");
      status = STATUS_ERROR;
    }
  }
...
{
  fprintf(stderr,
          "Usage: %s {-hv} -o profile.xml sample_directory ...\n"
          ...
          " -s | --symbols -- display symbols read from binary files\n"
          " -B | --validate-symbol-lookup -- verify symbols found by binary search\n"
...
Application::run(int argc, char** argv)
{
  ...
  if (m_profile_data_pathnames.empty())
  {
    if (m_verbose)
    {
      printf("No profiling data pathnames specified, adding default path: %s\n",
             DEFAULT_OPROFILE_DATA_PATHNAME.c_str());
    }

    m_profile_data_pathnames.add(DEFAULT_OPROFILE_DATA_PATHNAME);
  }
  ...
  if (Application::show_progress())
  {
    printf("Processing profile data archives...\n");
    fflush(stdout);
  }

  // Iterate over profile data directories.
  int archives_found = 0;
  FOR_EACH(const_iterator, it, Array<Pathname>, m_profile_data_pathnames)
  {
    const Pathname& pathname = *it;

    if (! pathname.is_directory())
    {
      fprintf(stderr, "Warning: the specified OProfile archive doesn't exist or isn't a directory: %s\n",
                      pathname.c_str());
      continue;
    }
    ...
    // Allow user to specify the parent directory of a copy of the /var/lib/oprofile tree.
    Pathname archive_subdir = Pathname(pathname, DEFAULT_OPROFILE_DATA_PATHNAME);
    if (archive_subdir.is_directory()) {
      archive = archive_subdir;
    }

    if (m_verbose || Application::show_progress())
    {
      printf("Processing OProfile archive: %s\n", archive.c_str());
      fflush(stdout);
    }

    // count sample files and display count found
    if (Application::show_progress()) {
      m_sample_files_count = 0;
      printf("Counting sample files in archive...\n");
      fflush(stdout);

      process_archive_directory(archive, &Application::count_sample_file);

      printf("%i sample files found.\n", m_sample_files_count);
      fflush(stdout);
    }

    // now process them for real
    if (Application::show_progress()) {
      printf("Processing sample files in archive...\n");
      print_current_time();
      fflush(stdout);
      m_sample_files_processed = 0;
    }

    process_archive_directory(archive, &Application::process_sample_file);

    if (Application::show_progress())
    {
      printf("%i sample files processed.\n", m_sample_files_processed);
      print_current_time();
      fflush(stdout);
    }
  }
  ...
  if (archives_found == 0)
  {
    fprintf(stderr, "No profile archives found, cannot generate XML file.\n");
    status = STATUS_ERROR;
    return status;
  }
 ...
 */
void
Application::process_archive_directory(const Pathname& archive,
                                       void (Application::*fn) (const Pathname& samplefile))
{
  process_archive_directory(archive, archive, fn);
}

/** Walks specified archive directory tree. */
void
Application::process_archive_directory(const Pathname& archive, const Pathname& pathname,
                                       void (Application::*fn) (const Pathname& samplefile))
{
  PathnameArray children;
  if (pathname.get_child_pathnames(children))
  {
    FOR_EACH(const_iterator, it, PathnameArray, children)
    {
      const Pathname& child = *it;
      if (child.is_directory()) {
        process_archive_directory(archive, child, fn);
      }
      else {
        process_archive_file(archive, child, fn);
      }
    }
  }
}

/** Processes archive file. */
void
Application::process_archive_file(const Pathname& archive, const Pathname& pathname,
                                  void (Application::*fn) (const Pathname& samplefile))
{
  // Apply processing function to pathname.
  ((*this).*fn)(pathname);
}

/** Counts sample files. */
void
Application::count_sample_file(const Pathname& pathname)
{
  ++m_sample_files_count;
  if ((m_sample_files_count % 1000) == 0) {
    printf("%i files seen...\n", m_sample_files_count);
  }
}
...
void
Application::process_sample_file(const Pathname& pathname)
{
  if (Application::show_progress()) {
    ++m_sample_files_processed;
    if ((m_sample_files_processed % 1000) == 0) {
      printf("%i of %i files processed...\n", m_sample_files_processed, m_sample_files_count);
      fflush(stdout);
    }
  }

  // NOTE: Hard-code session number to 1 until we decide
  // to add support for multiple-session processing.
  SampleFile samplefile(pathname, 1, m_symbolfiles);
  if (! samplefile.is_valid()) {
    return;
  }

  if (m_verbose)
  {
    printf("Processing OProfile sample file: %s\n", pathname.c_str());
  }

  // Cache event for this sample file, get id
  const std::string hardware_event_name = samplefile.get_event_name();
  const long interval = samplefile.get_event_interval();
  const int  mask     = samplefile.get_event_mask();
  int event_id = cache_event(hardware_event_name, interval, mask);

  // is it a callgraph file ("from->to" instead of just "from")?
  bool is_cg = samplefile.is_callgraph_file();

  // Cache binary paths used, get ids

  const std::string& executable_pathname = samplefile.get_executable_pathname();
  int binary_id = cache_binary(executable_pathname);

  const std::string& from_pathname = samplefile.get_from_pathname().to_string();
  int from_module_id = cache_binary(from_pathname);
  Binary* from_module = get_binary(from_module_id);

  const std::string& to_pathname = (!is_cg) ? from_pathname  : samplefile.get_to_pathname().to_string();
  int to_module_id               = (!is_cg) ? from_module_id : cache_binary(to_pathname);
  Binary* to_module              = (!is_cg) ? from_module    : get_binary(to_module_id);

  // Cache task (binary/pid/tid), get id
  int pid = to_int(samplefile.get_pid());
  int tid = to_int(samplefile.get_tid());
  int task_id = cache_task(pid, tid, binary_id);

  // Get CPU number, used for samples below.
  int cpu_id = to_int(samplefile.get_cpu_id());

  // Now we can iterate over the samples in this file.
  FOR_EACH(const_iterator, it, SampleArray, samplefile.get_samples())
  {
    const Sample& sample = *it;

    const SampleLocation& from_location =
      sample.from_location();
    const std::string& from_address =
      from_location.get_address();
    const Symbol* from_symbol =
      from_location.get_symbol();
    const std::string from_symbol_name =
      (from_symbol != NULL) ? from_symbol->get_name() : "";
    bfd_vma from_symbol_start =
      (from_symbol != NULL) ? from_symbol->get_start_vma() : 0 ;
    bfd_vma from_symbol_size =
      (from_symbol != NULL) ? from_symbol->get_size() : 0 ;
    bfd_vma from_symbol_end =
      (from_symbol != NULL) ? from_symbol->get_end_vma() : 0 ;
    const std::string from_source_file =
      from_location.get_source_pathname().to_string();
    const int from_source_line =
      from_location.get_source_line();

    int from_symbol_id = (from_symbol == NULL) ? 0 :
      cache_symbol(from_module_id,
                   from_module,
                   from_symbol_name,
                   from_symbol_start,
                   from_symbol_size,
    ...
    int from_location_id =
      cache_location(from_module_id,
                     from_address,
                     from_symbol_id,
                     from_source_file,
                     from_source_line);

    const SampleLocation& to_location = (!is_cg) ? from_location :
      sample.to_location();
    const std::string& to_address = (!is_cg) ? from_address :
      to_location.get_address();
    const Symbol* to_symbol = (!is_cg) ? from_symbol :
      to_location.get_symbol();
    const std::string to_symbol_name = (!is_cg) ? from_symbol_name :
      ((to_symbol != NULL) ? to_symbol->get_name() : "");
    bfd_vma to_symbol_start = (!is_cg) ? from_symbol_start :
      (to_symbol != NULL) ? to_symbol->get_start_vma() : 0 ;
    bfd_vma to_symbol_size = (!is_cg) ? from_symbol_size :
      (to_symbol != NULL) ? to_symbol->get_size() : 0 ;
    bfd_vma to_symbol_end = (!is_cg) ? from_symbol_end :
      (to_symbol != NULL) ? to_symbol->get_end_vma() : 0 ;
    const std::string to_source_file = (!is_cg) ? from_source_file :
      to_location.get_source_pathname().to_string();
    const int to_source_line = (!is_cg) ? from_source_line :
      to_location.get_source_line();

    int to_symbol_id = (!is_cg) ? from_symbol_id : (to_symbol == NULL) ? 0 :
      cache_symbol(to_module_id,
                   to_module,
                   to_symbol_name,
                   to_symbol_start,
                   to_symbol_size,
    ...
    int to_location_id = (!is_cg) ? from_location_id :
      cache_location(to_module_id,
                     to_address,
                     to_symbol_id,
                     to_source_file,
                     to_source_line);
    ...
    // An OProfile sample captures either:
    // (1) non-cg file: a single "self" count at a given location.
    // (2) cg file:     a "transition" count from a caller location to a callee location,
    //                  basically the number of times backtracking from "self" samples
    //                  encountered this from->to "arc" in the call stack.

    // So the resulting SampleData entries will have either:
    // (1) a single stack "frame" with the self count
    // (2) a pair of stack "frames":
    //     - a "to" frame with a zero count (since the "self" count is covered above)
    //     - a "from" frame with the transition count


    if (! is_cg) {
      sampledata->add_frame(from_location_id, value);
    }
    else {
      sampledata->add_frame(to_location_id, 0);
      sampledata->add_frame(from_location_id, value);
    }
  }
