...
void
ftp_init(cmd_obj_t* cmd_obj)
{
}


int
ftp_parse(cmd_obj_t* cmd_obj)
{
  int opt;
  ...
  while ((opt = getopt_long_only(cmd_obj->argc, cmd_obj->argv, "1234:5:",
         long_options, NULL)) != -1)
  {
    switch (opt)
    {
...
ftp_modify(cmd_obj_t* cmd_obj)
{
  ...
  add_param(cmd_obj, "ftpget", NULL);

  if (usrname)
  {
    add_param(cmd_obj, "-u", NULL);
    add_param(cmd_obj, usrname, NULL);
  }

  if (passwd)
  {
    add_param(cmd_obj, "-p", NULL);
    add_param(cmd_obj, passwd, NULL);
  }

  add_param(cmd_obj, host, NULL);
  ...
  file = locFile ? locFile : remFile;

  locFileWithPath = add_param(cmd_obj, path, file, NULL);  // local file
  add_param(cmd_obj, remFile, NULL);

  set_err_data(cmd_obj, "Couldn't download the image.\n");
...
void
ftp_error(const cmd_obj_t* const cmd_obj)
{
...
ftp_help()
{
  ...
  printf("Usage: ftp <host> [-u username] [-p passwd] <remotefile> "
         "<%s> [localfile]\n", dev_string);
}
