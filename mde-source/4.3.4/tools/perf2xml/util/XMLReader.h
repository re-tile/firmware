 ...
 */
std::string
xml_encode(const std::string& text);
 ...
 */
std::string
xml_decode(const std::string& text);
...
public:
  ...
  /** Constructor */
  XMLTag(Type type, const std::string& name) :
...
class XMLReaderBase
{
  // --- static methods ---
public:
  static XMLDocument* read(const Pathname& pathname, std::string& errors)
  {
    ...
    if (! fin)
    {
      ...
      errors += "Reason: (" + to_string(errno) + ") " +
        to_string(strerror(errno)) + "\n";
    }
    else
    {
      // load XML document
...
protected:
  ...
  void add_error(std::string& errors, const std::string& error) const
  {
  ...
  /** Appends error to error list, adding current line/char info. */
  void add_error(std::string& errors, const char* error) const
  {
    errors += "Line " + to_string(m_lineNumber) + ", column "
      + to_string(m_column) + ": ";
...
public:
   ...
   */
  void read_xml_content_text(std::string& result)
  {
    result.clear();
    int c;
    while ((c = peek()) >= 0)
   ...
   */
  bool read_xml_element_text(std::string& result, std::string& errors)
  {
    bool okay = true;
    result.clear();
    skip_whitespace();
    ...
    else
    {
      ...
      while ((c = peek()) >= 0)
      {
        ...
        if (c == '<')
        {
          add_error(errors, "While reading XML tag, found unexpected '<' "
                            "-- preceding tag was not properly closed.");
          okay = false;
          break;
        }
        else
  ...
  XMLTag* parse_xml_element_text(std::string& tag, std::string& errors)
  {
    XMLTag* result = NULL;
    XMLTag::Type type;
    type = XMLTag::UNKNOWN;
    ...
    while (! found_tag_end)
    {
      ...
      while (! found_tag_end && (x < len))
      {
        ...
        while (! found_tag_end && (x < len))
        {
          ...
          {
            // unexpected random garbage
            add_error(errors, "While reading tag attribute name, "
                              "found character '" + to_string((char) c) + "'");
            okay = false;
            break;
          }
          x++;
  ...
  XMLTag* read_tag(std::string& errors)
  {
    std::string text;
...
public:
   ...
   */
  XMLDocument* read(std::string& errors)
  {
    XMLDocument *result = NULL;
    ...
    while (true)
    {
      ...
      else
      {
        ...
        else if (tag_type == XMLTag::CLOSE_TAG)
        {
          if (currentElement == NULL)
          {
            add_error(errors, "Encountered close tag '" + tag_name + "' "
                              "with no corresponding open tag.");
            delete tag;
            break;
          }
          else if (currentElement->get_name() != tag_name)
          {
            add_error(errors, "Encountered close tag '" + tag_name +
                      "', but " + "was expecting close tag for '" +
                      currentElement->get_name() + "'.");
            delete tag;
            break;
          }
    ...
    // and return it
    if (errors.empty() && document != NULL && document->get_root() != NULL)
    {
      result = document;
    }
    else
