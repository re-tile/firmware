...
{
  std::string result;
  FOR_EACH(const_iterator, i, std::string, text)
  {
    char c = *i;
    switch (c)
    {
 ...
 *  (i.e. "2&lt;3" -> "2<3")
 */
std::string xml_decode(const std::string& text)
{
  std::string result;
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          std::string entity = text.substr(amp, semi-amp+1);
          if (entity == "&quot;")
          {
            result += "\"";
          }
          ...
          {
            std::string char_code = entity.substr(2,entity.size()-3);
