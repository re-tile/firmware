...
__gxio_dma_queue_reserve(__gxio_dma_queue_t *dma_queue,
                         unsigned int num, bool wait, bool completion)
{
  uint64_t slot;
  ...
  int64_t old =
    __insn_fetchaddgez(&dma_queue->credits_and_next_index, modifier);

  if (__unlikely(old + modifier < 0))
  {
    /*
    ...
    old = __insn_fetchaddgez(&dma_queue->credits_and_next_index, modifier);
    if (old + modifier < 0)
    {
      if (wait)
        old = __gxio_dma_queue_wait_for_credits(dma_queue, modifier);
      else
  ...
  if (completion)
  {
     ...
     */
    uint64_t complete;
    complete = arch_atomic_access_once(dma_queue->hw_complete_count);
   ...
   */
  if (__unlikely(((old + num) & 0xff) < num))
  {
    __gxio_dma_queue_update_credits(dma_queue);
