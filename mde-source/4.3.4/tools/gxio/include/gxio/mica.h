...
#include <arch/mica_comp_ctx_user.h>
 ...
 * shim will use raw deflate format, the fastest compression level
...
/** Remove a page from the context's IOTLB.
 *
 * @param context An initialized MiCA context.
 * @param page Starting VA of a contiguous memory page, must be page-aligned.
 * @return Zero on success, ::GXIO_ERR_IOTLB_ENTRY if mapping was not in
 * the IOTLB.
 */
extern int
 ...
 *
 * @param context An initialized MiCA context.
 * @param cpu CPU number of interrupt target tile.
 * @param e Specific event which will be set in the IPI0 register when
 * the interrupt occurs.
 * @return Zero on success, GXIO_ERR_INVAL if params are out of range.
 */
extern int
gxio_mica_cfg_completion_interrupt(gxio_mica_context_t* context,
                                   int cpu, int e);
 ...
 */
void
gxio_mica_memcpy_start(gxio_mica_context_t* context, void* dst,
                       void* src, int length);
