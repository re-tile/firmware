...
#if 0
#define TRACE(...) printf("pka: " __VA_ARGS__)
#else
#define TRACE(...)
#endif
...
static int
setup_rings(gxio_mica_pka_context_t* context, gxio_mica_pka_ring_config_t* rc)
{
  int ring_mem_size;
  int num_rings;
  int window_ptr;
  ...
  for (i = 0; i < num_rings; i++)
  {
    ...
    ring->num_elems = ring_mem_size / (sizeof(struct pka_hw_cmd_desc) +
                                       rc->ring_elem_size[i]);
    ring->elem_size = rc->ring_elem_size[i];
    ring->cmd_base = ring->cmd_head = ring->cmd_tail = window_ptr;
    ring->data_base = ring->data_head = ring->data_tail = window_ptr +
      (ring->num_elems * sizeof(struct pka_hw_cmd_desc));
    window_ptr += ring_mem_size;
    TRACE("ring %d: num_elems = %d, elem_size = 0x%x, cmd_base = 0x%x, "
          "data_base = 0x%x\n", i, ring->num_elems, ring->elem_size,
          ring->cmd_base, ring->data_base);
  }
  ...
  for (i = 0; i < num_rings; i++)
  {
    ...
    __gxio_mmio_write(pka_regs_base_addr +
                      MICA_CRYPTO_ENG_CMMD_RING_BASE_0 + ((i * 16) << 1),
                      context->ring[i].cmd_base);
    __gxio_mmio_write(pka_regs_base_addr +
                      MICA_CRYPTO_ENG_RSLT_RING_BASE_0 + ((i * 16) << 1),
                      context->ring[i].cmd_base);
    __gxio_mmio_write(pka_regs_base_addr +
                      MICA_CRYPTO_ENG_RING_SIZE_0 + ((i * 4) << 1),
                      context->ring[i].num_elems - 1);
    __gxio_mmio_write(pka_regs_base_addr +
                      MICA_CRYPTO_ENG_RING_RW_PTRS_0 + ((i * 4) << 1), 0);
  }
...
int
gxio_mica_pka_init(gxio_mica_pka_context_t* context, int mica_index,
...
static void
write_to_pka_window_ram(gxio_mica_pka_context_t* context,
                        ...
                        int len_in_4byte_words)
{
  ...
  for (i = 0; i < (len / sizeof(uint32_t)); i = i + 2)
  {
    ...
    // Skip the read of the last second 4-byte word if
    // len_in_4byte_words is odd.
    if ((len_in_4byte_words & 1) && (i + 1 == (len / sizeof(uint32_t))))
      wd = (uint64_t)src_addr[i];
    else
...
static void
read_from_pka_window_ram(gxio_mica_pka_context_t* context,
                         ...
                         int len_in_4byte_words)
{
  ...
  for (i = 0; i < (len / sizeof(uint32_t)); i = i + 2)
  {
    uint64_t wd;
    char* mmio_addr = context->mmio_data_base + offset + i*4;
    TRACE("WINDOW_RAM: reading from addr 0x%lx...", (long)mmio_addr);
    wd = __gxio_mmio_read(mmio_addr);

    // Issue wh64 if the destination address spans over a L2 cacheline.
    if ((((unsigned long) &dst_addr[i]) & (CHIP_L2_LINE_SIZE() - 1)) == 0 &&
        (i + CHIP_L2_LINE_SIZE()/sizeof(uint32_t)) <= (len / sizeof(uint32_t)))
    {
      __insn_wh64(&dst_addr[i]);
    }

    dst_addr[i] = (uint32_t)wd;
    // Skip the last write of the second 4-byte word
    // if len_in_4byte_words is odd.
    if (!((len_in_4byte_words & 1) && (i + 1 == (len / sizeof(uint32_t)))))
...
static
void debug_dump_hw_cmd_desc(struct pka_hw_cmd_desc* cmd)
{
  TRACE("hw cmd descriptor at %p\n", cmd);
  TRACE("ptr_a = 0x%08x\n", cmd->ptr_a);
  TRACE("ptr_b = 0x%08x\n", cmd->ptr_b);
  TRACE("ptr_c = 0x%08x\n", cmd->ptr_c);
  TRACE("ptr_d = 0x%08x\n", cmd->ptr_d);
...
// command.
static void
write_cmd_to_ring(gxio_mica_pka_context_t* context, int ring,
                  struct pka_hw_cmd_desc* cmd)
{
  ...
  debug_dump_hw_cmd_desc(cmd);

  TRACE("kicking the queue\n");
  // Increment command count
  __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_COMMAND_COUNT_0 +
                    ring_reg_offset, 1);

  // FIXME do this more efficiently
  if (((context->ring[ring].cmd_tail - context->ring[ring].cmd_base) >=
       (sizeof(struct pka_hw_cmd_desc) * (context->ring[ring].num_elems - 1))))
...
static int
read_res_from_ring(gxio_mica_pka_context_t* context, int ring,
                   struct pka_hw_res_desc* res)
{
  ...
  __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_RESULT_COUNT_0 +
                                       ring_reg_offset, 1);
...
static int
setup_hw_cmd_ptrs(gxio_mica_pka_context_t* context, int ring,
                  pka_cmd_desc_t* cmd, struct pka_hw_cmd_desc* hw_cmd)
{
  pka_ring_t* pring = &context->ring[ring];

  hw_cmd->ptr_a = context->ring[ring].data_tail;
  hw_cmd->ptr_b = hw_cmd->ptr_a + (ROUND_UP_2(cmd->vec_a.len) * 4);

  if (cmd->command == 5)
  {
    // Special care for division, which has 2 result vectors, C and D.
    // and the divisor's most significant 4-byte must be non zero.

    int i = 0;

    for (i = 0; i < cmd->vec_b.len; i++)
    {
      if (*((uint32_t *)(uintptr_t)
            (cmd->vec_b.va + sizeof(uint32_t) * (cmd->vec_b.len - 1 - i))))
        break;
    }

    if (i < cmd->vec_b.len)
    {
      // Skip the first i double words in vector B.
      TRACE("pka: reduce divide vec B %x\n", i);
      hw_cmd->len_b -= i;
      hw_cmd->ptr_c = hw_cmd->ptr_b + ROUND_UP_2(cmd->vec_b.len) * 4;
      hw_cmd->ptr_d = hw_cmd->ptr_c + ROUND_UP_2(cmd->vec_b.len) * 4;
    }
    else
      // The divisor is zero.
      return GXIO_ERR_INVAL;
  ...
  {
    hw_cmd->ptr_c = hw_cmd->ptr_b + (ROUND_UP_2(cmd->vec_b.len) * 4);
    hw_cmd->ptr_d = hw_cmd->ptr_c + (ROUND_UP_2(cmd->vec_c.len) * 4);
  }

  hw_cmd->ptr_e = hw_cmd->ptr_d + (ROUND_UP_2(cmd->vec_d.len) * 4);

  if ((hw_cmd->ptr_e - hw_cmd->ptr_a + (ROUND_UP_2(cmd->vec_e.len) * 4)) >
      pring->elem_size)
  {
...
int
gxio_mica_pka_command_slots_available(gxio_mica_pka_context_t* context,
                                      int ring)
{
  return context->ring[ring].num_elems - context->ring[ring].cmd_slots_in_use;
}


int
gxio_mica_pka_put_command_on_ring(gxio_mica_pka_context_t* context,
                                  int ring, pka_cmd_desc_t* cmd)
{
  ...
  if (cmd->vec_a.len)
  {
    TRACE("copying ptr_a from va %#llx to offset 0x%x\n",
          (unsigned long long)cmd->vec_a.va,
          (unsigned int)hw_cmd.ptr_a);
    write_to_pka_window_ram(context, hw_cmd.ptr_a,
                            (uint32_t *)(uintptr_t)cmd->vec_a.va,
                            cmd->vec_a.len);
  }
  if (cmd->vec_b.len)
  {
    TRACE("copying ptr_b from va %#llx to offset 0x%x\n",
          (unsigned long long)cmd->vec_b.va,
          (unsigned int)hw_cmd.ptr_b);
    write_to_pka_window_ram(context, hw_cmd.ptr_b,
                            (uint32_t *)(uintptr_t)cmd->vec_b.va,
                            cmd->vec_b.len);
  }
  if (cmd->vec_c.len)
  {
    TRACE("copying ptr_c from va %#llx to offset 0x%x\n",
          (unsigned long long)cmd->vec_c.va,
          (unsigned int)hw_cmd.ptr_c);
    write_to_pka_window_ram(context, hw_cmd.ptr_c,
                            (uint32_t *)(uintptr_t)cmd->vec_c.va,
                            cmd->vec_c.len);
  }
  if (cmd->vec_d.len)
  {
    TRACE("copying ptr_d from va %#llx to offset 0x%x\n",
          (unsigned long long)cmd->vec_d.va,
          (unsigned int)hw_cmd.ptr_d);
    write_to_pka_window_ram(context, hw_cmd.ptr_d,
                            (uint32_t *)(uintptr_t)cmd->vec_d.va,
                            cmd->vec_d.len);
  }
  if (cmd->vec_e.len)
  {
    TRACE("copying ptr_e from va %#llx to offset 0x%x\n",
          (unsigned long long)cmd->vec_e.va,
          (unsigned int)hw_cmd.ptr_e);
    write_to_pka_window_ram(context, hw_cmd.ptr_e,
                            (uint32_t *)(uintptr_t)cmd->vec_e.va,
                            cmd->vec_e.len);
  }
...
int
gxio_mica_pka_results_available(gxio_mica_pka_context_t* context,
                                int ring)
{
  int ring_reg_offset = ring * 8;
  int result_count =  __gxio_mmio_read(context->mmio_regs_base +
...
int
gxio_mica_pka_get_result_from_ring(gxio_mica_pka_context_t* context,
                                   int ring, pka_res_desc_t* res)
{
  ...
  if (res->vec_a.len)
  {
    TRACE("copying res ptr_a from offset 0x%x to va %#llx\n",
          (unsigned int)pres->ptr_a,
          (unsigned long long)res->vec_a.va);
    read_from_pka_window_ram(context, pres->ptr_a,
                             (uint32_t *)(uintptr_t)res->vec_a.va,
                             res->vec_a.len);
  }
  if (res->vec_b.len)
  {
    TRACE("copying res ptr_b from offset 0x%x to va %#llx\n",
          (unsigned int)pres->ptr_b,
          (unsigned long long)res->vec_b.va);
    read_from_pka_window_ram(context, pres->ptr_b,
                             (uint32_t *)(uintptr_t)res->vec_b.va,
                             res->vec_b.len);
  }
  if (res->vec_c.len)
  {
    TRACE("copying res ptr_c from offset 0x%x to va %#llx\n",
          (unsigned int)pres->ptr_c,
          (unsigned long long)res->vec_c.va);

    if (res->command == 5)
      // Special case for division, only read len_b words.
      read_from_pka_window_ram(context, pres->ptr_c,
                               (uint32_t *)(uintptr_t)res->vec_c.va,
                               (res->vec_c.len > res->hw_len_b) ?
                               res->hw_len_b : res->vec_c.len);
    else
      read_from_pka_window_ram(context, pres->ptr_c,
                               (uint32_t *)(uintptr_t)res->vec_c.va,
                               res->vec_c.len);
  ...
  if (res->vec_d.len)
  {
    TRACE("copying res ptr_d from offset 0x%x to va %#llx\n",
          (unsigned int)pres->ptr_d,
          (unsigned long long)res->vec_d.va);
    read_from_pka_window_ram(context, pres->ptr_d,
                             (uint32_t *)(uintptr_t)res->vec_d.va,
                             res->vec_d.len);
  }
...
void
gxio_mica_pka_read_interrupts(gxio_mica_pka_context_t* context,
                              uint32_t* interrupts)
{
  *interrupts = __gxio_mmio_read(context->mmio_regs_base +
                                 MICA_CRYPTO_ENG_AIC_ENABLED_STAT) & 0xff;
}

void
gxio_mica_pka_reset_interrupts(gxio_mica_pka_context_t* context,
                              uint32_t interrupts)
{
...
int
gxio_mica_pka_trng_init(gxio_mica_pka_trng_context_t* context, int mica_index)
{
  char file[32];
  int fd;

  snprintf(file, sizeof(file), "/dev/iorpc/trng%d", mica_index);
  ...
  // Map in the control registers and the data memory in one shot,
  // then assign the base registers to the proper offsets.
  // FIXME: this also maps in the TRNG registers.  We really
  // should map the PKA control registers separately from the
  // window RAM and Tilera control registers to avoid the TRNG
  // registers, for a little more protection.
  context->mmio_regs_base =
    mmap(NULL, HV_PKA_REGS_MMIO_SIZE + HV_PKA_DATA_MMIO_SIZE,
         PROT_READ | PROT_WRITE, MAP_SHARED, context->fd, 0);
  if (context->mmio_regs_base == MAP_FAILED)
  {
    TRACE("mmap failed.  err = %s\n", strerror(errno));
    close(context->fd);
    return -errno;
...
int
gxio_mica_pka_trng_data_present(gxio_mica_pka_trng_context_t* context)
{
...
static int inline
trng_check_shutdown_oflo(gxio_mica_pka_trng_context_t* context)
{
  uint64_t reg =__gxio_mmio_read(context->mmio_regs_base +
                                   MICA_CRYPTO_ENG_TRNG_STATUS);
  ...
  {
    uint64_t current_cycle_count = get_cycle_count();
    ...
    if (reg)
    {
      ...
      uint64_t enabled = __gxio_mmio_read(context->mmio_regs_base +
                                          MICA_CRYPTO_ENG_TRNG_FROENABLE);
      __gxio_mmio_write(context->mmio_regs_base +
                        MICA_CRYPTO_ENG_TRNG_FRODETUNE, reg);
      __gxio_mmio_write(context->mmio_regs_base +
                        MICA_CRYPTO_ENG_TRNG_FROENABLE, enabled | reg);
    }

    // Reset the error.
    __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_TRNG_ALARMMASK,
                      0);
    __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_TRNG_ALARMSTOP,
                      0);
    __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_TRNG_STATUS,
    ...
    if (context->last_error_cycle &&
        (current_cycle_count - context->last_error_cycle < 1000000000))
    {
      reg = __gxio_mmio_read(context->mmio_regs_base +
...
int
gxio_mica_pka_trng_read32(gxio_mica_pka_trng_context_t* context,
                          uint32_t* data)
{
  ...
  *data = __gxio_mmio_read(context->mmio_regs_base +
  ...
  __gxio_mmio_write(context->mmio_regs_base + MICA_CRYPTO_ENG_TRNG_INTACK, 1);
...
int
gxio_mica_pka_trng_read64(gxio_mica_pka_trng_context_t* context,
                          uint64_t* data)
{
  if (trng_check_shutdown_oflo(context))
    return -1;
